...
public class UnlockRepository : IUnlockRepository
{
    private readonly OneDcDbContext _db;
    public UnlockRepository(OneDcDbContext db) => _db = db;
    ...
    private IQueryable<TimesheetEntry> BaseQuery(DateOnly from, DateOnly to, Guid? projectId, Guid? userId)
    {
        var q = _db.TimesheetEntries
            .Where(t => t.WorkDate >= from && t.WorkDate <= to &&
                        t.Status == TimesheetStatus.LOCKED);
        if (projectId.HasValue) q = q.Where(t => t.ProjectId == projectId.Value);
        if (userId.HasValue)    q = q.Where(t => t.UserId == userId.Value);
        return q;
    }

    public Task<int> CountLockedAsync(DateOnly from, DateOnly to, Guid? projectId, Guid? userId)
        => BaseQuery(from, to, projectId, userId).CountAsync();

    public Task<int> UnlockLockedAsync(DateOnly from, DateOnly to, Guid? projectId, Guid? userId)
        => BaseQuery(from, to, projectId, userId)
            .ExecuteUpdateAsync(s => s
                .SetProperty(t => t.Status, TimesheetStatus.APPROVED)
