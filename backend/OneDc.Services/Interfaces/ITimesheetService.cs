using OneDc.Domain.Entities;

namespace OneDc.Services.Interfaces;

public record TimesheetCreateDto(Guid ProjectId, DateOnly WorkDate, decimal Hours, string? Description, string? TicketRef, Guid? TaskId, TaskType TaskType);
public record TimesheetUpdateDto(decimal Hours, string? Description, string? TicketRef, Guid? TaskId, TaskType? TaskType);

public interface ITimesheetService
{
    Task<IEnumerable<TimesheetEntry>> GetMineAsync(Guid userId, DateOnly from, DateOnly to);
    Task<TimesheetEntry> CreateDraftAsync(Guid userId, TimesheetCreateDto dto);
    Task<TimesheetEntry> UpdateDraftAsync(Guid userId, Guid entryId, TimesheetUpdateDto dto);
    Task<TimesheetEntry> SubmitAsync(Guid userId, Guid entryId);
    Task DeleteAsync(Guid userId, Guid entryId);
    
    // Admin methods
    Task<IEnumerable<TimesheetEntry>> GetAllAsync(DateOnly from, DateOnly to);
    Task<IEnumerable<TimesheetEntry>> GetForProjectAsync(Guid projectId, DateOnly from, DateOnly to);
}
